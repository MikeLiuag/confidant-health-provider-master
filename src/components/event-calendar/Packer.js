...
function buildEvent (column, left, width,durationType) {
...
function pack (columns, width, calculatedEvents,durationType) {
  ...
  for (var i = 0; i < colLength; i++) {
    ...
    for (var j = 0; j < col.length; j++) {
      ...
      calculatedEvents.push(buildEvent(col[j], L, W,durationType))
    }
  }
}

function populateEvents (events, screenWidth,durationType) {
  let lastEnd
  let columns
  let self = this
  ...
  events.forEach(function (ev, index) {
    if (lastEnd !== null && ev.start >= lastEnd) {
      pack(columns, screenWidth, calculatedEvents,durationType)
      columns = []
      lastEnd = null
    }
